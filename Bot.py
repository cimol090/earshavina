...
#roomlock
simlocks = []
file = open("locks.txt", "r") # read-only
for simi in file.readlines():
  if len(name.strip())>0: locks.append(name.strip())
f.close()
#Dlist
dlist = []
...
def newPoi():
 headers = {}
 headers['User-Agent'] = "Mozilla/5.0 (X11; Linux i686) AppleWebKit/537.17 (KHTML, like Gecko) Chrome/24.0.1312.27 Safari/537.17"
 req = urllib.request.Request("http://nekopoi.fun/", headers = headers)
 resp = urllib.request.urlopen(req).read().decode("utf-8").replace('\n','').replace('\r','').replace('\t','')
...
def serPoi(args):
 headers = {}
 headers['User-Agent'] = "Mozilla/5.0 (X11; Linux i686) AppleWebKit/537.17 (KHTML, like Gecko) Chrome/24.0.1312.27 Safari/537.17"
 req = urllib.request.Request("http://nekopoi.fun/?s="+""+"&post_type=anime".join(args.split()), headers = headers)
 resp = urllib.request.urlopen(req).read().decode("utf-8").replace('\n','').replace('\r','').replace('\t','')
 wa = re.findall('<h2><a href="(.*?)">(.*?)</a></h2>',resp)
 _title = re.search(r'<title[^>]*>([^<]+)</title>', resp).group(1)
 newset = list()
...
#Well, you can actually take a little time to look at it and learn something
  ...
  def onMessage(self, room, user, message):
   ...
   try:
    ...
    if message.body.startswith("brb")or message.body.startswith("BRB") and not user.name in blacklist:
     if self.getAccess(user) >= 1 or room.getLevel(user) > 0 and not user.name in blacklist:
      room.message (random.choice(["Jangan lama-lama yah @"+user.name+" ;)",]),True)
    if message.body.startswith("off")or message.body.startswith("OFF") and not user.name in blacklist:
     if self.getAccess(user) >= 1 or room.getLevel(user) > 0 and not user.name in blacklist:
    ...
    if message.body[0] in ["<"]:   
      ...
      ##npcmds
      if cmd == "npcmds":
        room.message("<f x12ff3399='1'>Nekopoi Commands :<f x12ff3399='1'><br/>1.npnew or npnew2 (New Update On Nekopoi.win)<f x12ff3399='1'><br/>2.npsr or npsr2(Nekopoi Search)<f x12ff3399='1'><br/>3.rech(Recommended Hentai)", True)
            ##bestH
      if cmd == "rech":
    ...
    ##check access and ignore
      ...
      if cmd == "lockstatus":
        if room.name in locks:
          roomlock = "<f x11FF0000='0'>Locked</f>"
        else:
          roomlock = "<f x1133FF33='0'>Unlocked</f>"
...
#### Invite
      ...
      ##cmds
        ...
        if user.name in archwizard and not user.name in owners and not user.name in admin and not user.name in whitelist:
          room.message("<br/>"+user.name+" Rank<f x12FF0000='Elephant'>[Fethmus] "+"<br/>"+"<f x12FF0000='Ravie'> Commands :<f x12FF0000='Elephant'><br/>ping , rb , rb2 , wl , cso , pfpic , prof or profile , gs(Google search) , gis(Google Image search) , yt(Youtube) , df(define) , udf(undefine) , fax , bc , sn(sendnote) , rn(readnote), mydict, nick , staff , setnick , mynick , seenick , profile , rank , myrank , ranker , myip , mc(MultiChat) , mc2(MultiChat) , lockstatus , bgimg , bgtime.<br/><f x12ff3399='1'>New Commands : npcmds(Nekopoi Commands) , gcmds(game)",True)
      ...
      ##testcmd
      if cmd == "npnew": room.message(newPoi(), True)
      if cmd == "npnew2": room.message(newPoi2(), True)
      if cmd == "bgtime": room.message(bgtime(args), True)
      if cmd == "npsr": room.message(serPoi(args), True)
      if cmd == "npsr2": room.message(serPoi2(args), True)
    ...
    ###### Define            
      ...
      #cmdnya
      if cmd == "simlock" and self.getAccess(user) >= 4:
           if args in simlocks:
            room.message("already Mute mode :|", True)
           else:
              simlocks.append(args)
              f.close()
              file = open("simlocks.txt", "w")
              file.write("\n".join(locks))
              file.close()
              room.message("I am Mute :|")
           if args == "":
            if room.name in simlocks:
              room.message("already muted :|")
            if room.name not in simlocks:
              simlocks.append(room.name)
              file = open("simlocks.txt", "w")
              file.write("\n".join(locks))
              file.close()
              room.message("Muted: %s" % args, True)
      if cmd == "simunlock" and self.getAccess(user) >= 4:
           if args in simlocks:
              simlocks.remove(args)
              file = open("simlocks.txt", "w")
              file.write("\n".join(locks))
              file.close()
              room.message("Unmuted: %s" % args, True)
           else: room.message("Lol")
           if args == "":
            if room.name in simlocks:
              room.message("I am unmuted :|")
            else:
              simlocks.remove(args)
              file = open("simlocks.txt", "w")
              file.write("\n".join(locks))
      ...
      ###lock
      if cmd == "lock" and self.getAccess(user) > 4:
          try:
           if args in locks:
            room.message("already locked. ;)")
            return
           if args in self.roomnames:
            if user.name in owners or user.name in admin or user.name not in archknight or user.name not in archwizard:
              locks.append(args)
              f.close()
              f = open("locks.txt", "w")
              f.write("\n".join(locks))
            ...
            else: room.message("Sorry,Only rank 2+ can do that")
           if args == "":
            if room.name in locks:
              room.message("already locked :)")
              return
          ...
          try:
           ...
           if not args:
             ...
             if rnm in locks:
               ...
               f.write("\n".join(locks))
               f.close()
               room.message("Unlocked: %s" % room.name, True)
             else:
               room.message("Already Unlocked")
           if args:
             rnm = room.name
             if rnm in locks and rnm in self.roomnames:
               locks.remove(rnm)
               f = open("locks.txt","w")
               f.write("\n".join(locks))
               f.close()
               ...
               room.message("Sudah di unlock")
          except:
           room.message("Error",True)
